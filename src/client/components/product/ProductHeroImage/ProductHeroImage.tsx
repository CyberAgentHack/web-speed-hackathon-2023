...
export const ProductHeroImage: FC<Props> = memo(({ product, title }) => {
  const thumbnailFileUrl = product.media.find(
    (productMedia) => productMedia.isThumbnail
  )?.file.filename + '?isHero=true' || 'data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==';
            ...
            <Anchor href={`/product/${product.id}`}>
              <div className={styles.container()}>
                <AspectRatio ratioHeight={9} ratioWidth={16}>
                  <img className={styles.image()} src={thumbnailFileUrl} />
                </AspectRatio>
